...
The application queries the Neo4j database for placeholder PubMedArticle nodes (i.e. nodes that have the needs_properties flag set to true)
...
def get_needs_properties_batch(n):
    ...
    """
    # Define the query
    query = "MATCH (p:PubMedArticle) WHERE p.needs_properties = TRUE RETURN p.pub_id limit " + str(n)
    ...
    while pmf.needs_properties_nodes_exist:
        ...
        if pubmed_articles is None or not pubmed_articles:
            ...
            for pubmed_id in pubmed_ids:
                pmf.negate_needs_props_and_refs(pubmed_id)
        else:
